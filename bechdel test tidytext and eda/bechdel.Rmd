...
#counts of movies that pass over each decade
movies %>%
...
#density of movies that pass or fail and their gross 
movies %>%
  filter(year >= 1990) %>%
...
#counts of each genre that pass the test, and their median gross
movie_genres %>%
  group_by(genre) %>%
  summarize_bechdel() %>%
  head(12) %>%
...
#each genre faceted, by proportion that pass and gross. also lumped by top 9
movie_genres %>%
...
#boxplot of genres by gross, and whether they pass or fail
movies %>%
  filter(fct_lump(genre, 9) != "Other") %>%
...
#linear model based on genres. one hot encoded where each genre is its own column
movies %>%
  filter(year >= 1990) %>%
  separate_rows(genre, sep = ", ") %>%
  mutate(value = 1) %>%
...
#very cool facet manipulation, can change titles and order
genre_reasons %>%
...
```{r}
#tokenize each word in the plot
movie_words <- movies %>%
  filter(!is.na(plot)) %>%
  unnest_tokens(word, plot) %>%
  anti_join(stop_words, by = "word")

#only keep top 50 most used words in all plots
movie_words %>%
