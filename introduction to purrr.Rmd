...
creating a linear model for each species. 
each species gets split into its own DF, then lm is run against each. 
a second map function is applied to each model to tidy it and extract the coeffieicents
then the unnesting can occur
```{r}
library(broom)
iris %>% 
  nest(-Species) %>% 
